...
- [x] **NYC Sticker-Pack Surprise picker is now a 2x2 tile grid.** The regular/super sticker-picking grids inside the hidden NYC Sticker-Pack Surprise (`StickerGrid` in `NYCSurpriseScreen`) were a 3-column CSS grid; changed to a 2-column grid (tiles 2x2, wrapping to additional rows below) with `overflowX: hidden` so the pack only ever scrolls up/down, never side-to-side, regardless of screen width. (Briefly tried a single-column stack first; settled on 2x2 tiling per follow-up feedback.)
